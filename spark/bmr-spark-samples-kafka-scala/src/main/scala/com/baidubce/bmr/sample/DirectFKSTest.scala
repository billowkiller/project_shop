...
object DirectFKSTest {
  ...
  def main(args: Array[String]) {
    if (args.length < 2) {
      System.err.println("Usage: DirectFKSTest <master> <topics>")
      System.exit(1)
    }
    
    val Array(master, topics) = args
    setup(master)

    val sparkConf = new SparkConf().setAppName("DirectFKSTest")
